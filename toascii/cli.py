...
from toascii import ConverterOptions, converters, gradients, Image as AsciiImage, Video as AsciiVideo, \
    FrameClearStrategy

CONVERTER_TYPE_OPTIONS = {
    c.__name__.lower(): c
    for c in map(lambda a: getattr(converters, a), dir(converters))
    if (isinstance(c, type) and issubclass(c, converters.BaseConverter) and c is not converters.BaseConverter)
}
...
def cb_converter(ctx: click.Context, param: click.Parameter, value: str) -> converters.BaseConverter:
...
@click.argument("converter", type=click.Choice(list(CONVERTER_TYPE_OPTIONS), case_sensitive=False), callback=cb_converter)
...
@click.option("--x-stretch", "--xstretch", type=click.FloatRange(min=0.0, min_open=True), default=1.0)
@click.option("--y-stretch", "--ystretch", type=click.FloatRange(min=0.0, min_open=True), default=1.0)
...
def toascii_command(**kwargs):
    ...
    converter_options = ConverterOptions(
        **{k: kwargs.pop(k) for k in list(kwargs) if k in ConverterOptions.schema()["properties"].keys()}
    )
