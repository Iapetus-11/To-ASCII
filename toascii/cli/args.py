...
class ArgDef(Generic[T]):
    ...
    def __init__(self, name: str, prefix: Union[str, int], converter: Callable[[str], T], optional: bool = False):
        self.name = name
        self.prefix = prefix
...
def ca_converter(value: str) -> converters.BaseConverter:
    opts = {
        c.__name__.lower(): c
        for c in
        map(lambda a: getattr(converters, a), dir(converters)) if (isinstance(c, type) and issubclass(c, converters.BaseConverter))
    }

    if (value := value.lower()) not in opts:
        raise ArgConverterException(f"invalid value: {value!r} (must be one of [{', '.join(map(lambda o: o.__name__.split('.')[-1], opts.values()))}])")
...
ARGS: Dict[Union[str, int], ArgDef] = {a.prefix: a for a in [
