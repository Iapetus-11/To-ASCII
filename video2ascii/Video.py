...
class Video:
    ...
    def asciify_row(self, executor, row):
        return (executor.map(self.asciify_pixel, (self, row,)),)

    def asciify_img(self, executor, img):
        return (executor.map(self.asciify_row, (self, executor, img,)),)
    ...
    def convert(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            while True:
                ...
                self.frames.append((executor.map(self.asciify_img, (self, executor, img)),))
